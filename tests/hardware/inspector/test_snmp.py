...
from .base import InspectorBaseTest as Base
...
class TestSNMPInspector(Base, test_base.TestCase):
    def setUp(self):
        super(TestSNMPInspector, self).setUp()
        self.inspector = snmp.SNMPInspector()
        self.host = network_utils.urlsplit("snmp://localhost")
        self.stubs.Set(self.inspector._cmdGen, 'getCmd', faux_getCmd)
        self.stubs.Set(self.inspector._cmdGen, 'nextCmd', faux_nextCmd)
    ...
    def test_get_cmd_error(self):
        self.stubs.Set(self.inspector, '_memory_total_oid', 'failure')
        ...
        self.assertRaises(snmp.SNMPException,
                          get_list,
                          self.inspector.inspect_memoryspace,
                          self.host)
